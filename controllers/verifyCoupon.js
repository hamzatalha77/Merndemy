...
const verifyCoupon = async (body) => {
  ...
  if (Coupon) {
    if (
      new Date().getTime() > Coupon.startDate.getTime() &&
      new Date().getTime() < Coupon.endDate.getTime()
    ) {
      if (body.total >= Coupon.thresholdAmount) {
        ...
        return {
          inRange: false,
          message: `Need to add more items worth ${
            Coupon.thresholdAmount - body.total
          } to the list for this coupon to be enabled.`
        }
      }
    } else {
      return {
